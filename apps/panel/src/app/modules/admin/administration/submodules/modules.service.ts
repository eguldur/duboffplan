...
export class SubModulesService
{
        ...
        Observable<{ pagination: ItemPagination; items: BaseItem[] }>
    {
        ...
        .valueChanges.pipe(
          map(({ data, loading }) => {
            this._pagination.next(data.submodulesPegination.pagination);
            this._items.next(data.submodulesPegination.items);
            this._counts.next(data.submodulesCount);
            return data;
          })
        );
    }
        ...
        return this.items$.pipe(
            ...
            this.apollo.mutate<any>({
              mutation: this._graphql.createItem,
                ...
                map((data: any) => {
                  this._items.next([data.data.newSubModules, ...items]);
                  this._item.next(data.data.newSubModules);
                  return data.data.newSubModules;
                })
              )
            )
          );
    ...
    updateitem(id: string, updateItem: BaseItem): Observable<BaseItem>
    {
            ...
            this.apollo.mutate<any>({
                ...
                .pipe(
                    ...
                    arrayForSort[index] = updatedItem.data.updateSubmodule;
                    this._items.next(arrayForSort);
                    this._item.next(updatedItem.data.updateSubmodule);
                    return updatedItem.data.updateSubmodule;
                    }),
