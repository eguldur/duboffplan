  ...
  styles: [
    `
      .${moduleConfig.type}-${moduleConfig.type2}-grid {
        grid-template-columns: 25px  minmax(256px, auto) 128px 200px 96px 96px 96px;
      }
    `,
  ],
...
export class SubModulesListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    private _itemService: SubModulesService,
  ...
  createItemForm(): void {
    this.selectedItemForm = this._formBuilder.group({
      ...
      isActive            : [false],
      siraNo              : [null, [Validators.required]],
  ...
  toggleDetails(itemId: string): void {
    ...
    this._itemService.getitemById(itemId).subscribe((product) => {
      ...
      this.selectedItemForm.controls["basemodule"].setValue(product?.basemodule?.id);
      this._changeDetectorRef.markForCheck()
    })
  }
  ...
  exportData(justSelected: boolean): void {
    ...
    this.apollo
      ...
      .subscribe(({ data, loading }) => {
        this.exportToexcel(data.submodulesPegination.items)
        this._changeDetectorRef.markForCheck()
      })
  }
