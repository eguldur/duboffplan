...
export class ModulesService
{
        ...
        Observable<{ pagination: ItemPagination; items: BaseItem[] }>
    {
        ...
        .valueChanges.pipe(
          map(({ data, loading }) => {
            this._pagination.next(data.basemodulesPegination.pagination);
            this._items.next(data.basemodulesPegination.items);
            this._counts.next(data.basemodulesCount);
            return data;
          })
        );
    }
        ...
        return this.items$.pipe(
            ...
            this.apollo.mutate<any>({
              mutation: this._graphql.createItem,
                ...
                map((data: any) => {
                  this._items.next([data.data.newBaseModules, ...items]);
                  this._item.next(data.data.newBaseModules);
                  return data.data.newBaseModules;
                })
              )
            )
          );
    ...
    updateitem(id: string, updateItem: BaseItem): Observable<BaseItem>
    {
            ...
            this.apollo.mutate<any>({
                ...
                .pipe(
                    ...
                    arrayForSort[index] = updatedItem.data.updateBasemodule;
                    this._items.next(arrayForSort);
                    this._item.next(updatedItem.data.updateBasemodule);
                    return updatedItem.data.updateBasemodule;
                    }),
