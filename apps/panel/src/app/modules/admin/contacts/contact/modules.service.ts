...
export class ModulesService
{
    ...
    get selects$(): Observable<any>
    {
        return this._selects.asObservable();
    }
    ...
    getSelects(): Observable<any>
    {
        ...
        .valueChanges.pipe(
          map(({ data, loading }) => {
            this._selects.next({phonetypes: data.phonetypes, socialmediaplatforms: data.socialmediaplatforms, developers: data.developers, unvans: data.unvans});
            return data;
          })
        );
    }
        ...
        Observable<{ pagination: ItemPagination; items: BaseItem[] }>
    {
        ...
        .valueChanges.pipe(
          map(({ data, loading }) => {
            this._pagination.next(data.itemsPegination.pagination);
            this._items.next(data.itemsPegination.items);
            this._counts.next(data.itemsCount);
            return data;
          })
        );
    }
        ...
        return this.items$.pipe(
            ...
            this.apollo.mutate<any>({
              mutation: this._graphql.createItem,
                ...
                map((data: any) => {
                  this._items.next([data.data.createNew, ...items]);
                  this._item.next(data.data.createNew);
                  return data.data.createNew;
                })
              )
            )
          );
    ...
    updateitem(id: string, updateItem: BaseItem, changeAvatar: boolean): Observable<BaseItem>
    {
            ...
            this.apollo.mutate<any>({
                ...
                .pipe(
                    ...
                    arrayForSort[index] = updatedItem.data.updateBaseSetting;
                    this._items.next(arrayForSort);
                    this._item.next(updatedItem.data.updateBaseSetting);
                    */
