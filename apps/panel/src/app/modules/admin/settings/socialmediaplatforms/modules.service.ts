...
export class ModulesService
{
        ...
        Observable<{ pagination: ItemPagination; items: BaseItem[] }>
    {
        ...
        .valueChanges.pipe(
          map(({ data, loading }) => {
            this._pagination.next(data.basesettingsPegination.pagination);
            this._items.next(data.basesettingsPegination.items);
            this._counts.next(data.basesettingsCount);
            return data;
          })
        );
    }
        ...
        return this.items$.pipe(
            ...
            this.apollo.mutate<any>({
              mutation: this._graphql.createItem,
                ...
                map((data: any) => {
                  this._items.next([data.data.newBaseSettings, ...items]);
                  this._item.next(data.data.newBaseSettings);
                  return data.data.newBaseSettings;
                })
              )
            )
          );
    ...
    updateitem(id: string, updateItem: BaseItem): Observable<BaseItem>
    {
            ...
            this.apollo.mutate<any>({
                ...
                .pipe(
                    ...
                    */
                    return updatedItem.data.updateBaseSetting;
                    }),
